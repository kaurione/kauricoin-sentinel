...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, kauricoind):
        golist = kauricoind.rpc_command('gobject', 'list')
        ...
        try:
            ...
            for item in golist.values():
                (go, subobj) = self.import_gobject_from_kauricoind(kauricoind, item)
    ...
    @classmethod
    def import_gobject_from_kauricoind(self, kauricoind, rec):
        import decimal
        import kauricoinlib
        import inflection
        ...
        # shim/kauricoind conversion
        object_hex = kauricoinlib.SHIM_deserialise_from_kauricoind(object_hex)
        objects = kauricoinlib.deserialise(object_hex)
        subobj = None
        ...
        # exclude any invalid model data from kauricoind...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from kauricoind, with every run
        ...
        # get/create, then sync payment amounts, etc. from kauricoind - Kauricoind is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(kauricoind)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from kauricoind! %s" % e)
            govobj.vote_delete(kauricoind)
            return (govobj, None)
    ...
    def vote_delete(self, kauricoind):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(kauricoind, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, kauricoind, signal, outcome):
        ...
        output = kauricoind.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = kauricoinlib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(kauricoind, signal)

    def sync_network_vote(self, kauricoind, signal):
        printdbg('\tsyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = kauricoind.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 kauricoin addr, non-multisig
            if not kauricoinlib.is_valid_kauricoin_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Kauricoin address for network [%s], returning False" % (self.payment_address, config.network))
                return False
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (kauricoinlib.blocks_to_seconds(superblockcycle) / 2) +
    ...
    def is_deletable(self):
        ...
        # TBD (item moved to external storage/KauricoinDrive, etc.)
        return False
    ...
    def prepare(self, kauricoind):
        try:
            object_hash = kauricoind.rpc_command(*self.get_prepare_command())
...
class Superblock(BaseModel, GovernanceClass):
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not kauricoinlib.is_valid_kauricoin_address(addr, config.network):
    ...
    def hash(self):
        import kauricoinlib
        return kauricoinlib.hashit(self.serialise())
...
class Watchdog(BaseModel, GovernanceClass):
    ...
    @classmethod
    def active(self, kauricoind):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at >= (now - kauricoind.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
        return resultset

    @classmethod
    def expired(self, kauricoind):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at < (now - kauricoind.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
    ...
    def is_expired(self, kauricoind):
        now = int(time.time())
        return (self.created_at < (now - kauricoind.SENTINEL_WATCHDOG_MAX_SECONDS))

    def is_valid(self, kauricoind):
        if self.is_expired(kauricoind):
            return False

        return True

    def is_deletable(self, kauricoind):
        if self.is_expired(kauricoind):
            return True
