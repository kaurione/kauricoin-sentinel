...
def is_valid_kauricoin_address(address, network='mainnet'):
    ...
    kauricoin_version = 140 if network == 'testnet' else 45
    ...
    try:
        ...
        # rescue from exception, not a valid Kauricoin address
        return False

    if (address_version != kauricoin_version):
        return False
...
# shims 'til we can fix the kauricoind side
def SHIM_serialise_for_kauricoind(sentinel_hex):
    ...
    # superblock => "trigger" in kauricoind
    if govtype == 'superblock':
        obj[0] = 'trigger'

    # kauricoind expects an array (even though there is only a 1:1 relationship between govobj->class)
    obj = [obj]

    # re-pack
    kauricoind_hex = serialise(obj)
    return kauricoind_hex


# shims 'til we can fix the kauricoind side
def SHIM_deserialise_from_kauricoind(kauricoind_hex):
    from models import TALCASHD_GOVOBJ_TYPES

    # unpack
    obj = deserialise(kauricoind_hex)

    # shim from kauricoind
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('kauricoin.conf')
