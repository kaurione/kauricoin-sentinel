...
class KauricoinConfig():

    @classmethod
    def slurp_config_file(self, filename):
        # read kauricoin.conf config but skip commented lines
        f = io.open(filename)
        lines = []
        for line in f:
            if re.match('^\s*#', line):
        ...
        # data is kauricoin.conf without commented lines
        data = ''.join(lines)
    ...
    def get_rpc_creds(self, data, network='mainnet'):
        # get rpc info from kauricoin.conf
        match = re.findall(r'rpc(user|password|port)=(.*?)$', data, re.MULTILINE)

        # python >= 2.7
        creds = {key: value for (key, value) in match}

        # standard Kauricoin defaults...
        default_port = 16060 if (network == 'mainnet') else 26061

        # use default port for network if not specified in kauricoin.conf
        if not ('port' in creds):
            creds[u'port'] = default_port

        # convert to an int if taken from kauricoin.conf
