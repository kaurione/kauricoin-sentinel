...
def has_kauricoin_conf():
    import config
    import io

    valid_kauricoin_conf = False

    # ensure kauricoin_conf exists & readable
    #
    # if not, print a message stating that Kauricoin Core must be installed and
    # configured, including JSONRPC access in kauricoin.conf
    try:
        f = io.open(config.kauricoin_conf)
        valid_kauricoin_conf = True
    except IOError as e:
        print(e)

    return valid_kauricoin_conf
...
def main():
    ...
    if not has_kauricoin_conf():
        print("KauricoinCore must be installed and configured, including JSONRPC access in kauricoin.conf")
        sys.exit(1)
