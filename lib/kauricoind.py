...
class KauricoinDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some kauricoind methods
    ...
    @classmethod
    def from_kauricoin_conf(self, kauricoin_dot_conf):
        from kauricoin_config import KauricoinConfig
        config_text = KauricoinConfig.slurp_config_file(kauricoin_dot_conf)
        creds = KauricoinConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = kauricoinlib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = kauricoinlib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = kauricoinlib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_kauricoind_proto_version_with_sentinel_ping)
