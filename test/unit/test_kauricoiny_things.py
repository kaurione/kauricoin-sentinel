...
def test_valid_kauricoin_address():
    from kauricoinlib import is_valid_kauricoin_address

    main = valid_kauricoin_address()
    test = valid_kauricoin_address('testnet')

    assert is_valid_kauricoin_address(main) is True
    assert is_valid_kauricoin_address(main, 'mainnet') is True
    assert is_valid_kauricoin_address(main, 'testnet') is False

    assert is_valid_kauricoin_address(test) is False
    assert is_valid_kauricoin_address(test, 'mainnet') is False
    assert is_valid_kauricoin_address(test, 'testnet') is True


def test_invalid_kauricoin_address():
    from kauricoinlib import is_valid_kauricoin_address

    main = invalid_kauricoin_address()
    test = invalid_kauricoin_address('testnet')

    assert is_valid_kauricoin_address(main) is False
    assert is_valid_kauricoin_address(main, 'mainnet') is False
    assert is_valid_kauricoin_address(main, 'testnet') is False

    assert is_valid_kauricoin_address(test) is False
    assert is_valid_kauricoin_address(test, 'mainnet') is False
    assert is_valid_kauricoin_address(test, 'testnet') is False
...
def test_deterministic_masternode_elections(current_block_hash, mn_list):
    from kauricoinlib import elect_mn
...
def test_parse_masternode_status_vin():
    from kauricoinlib import parse_masternode_status_vin
...
def test_hash_function():
    ...
    hex_hash = "%x" % kauricoinlib.hashit(sb_data_hex)
...
def test_blocks_to_seconds():
    ...
    assert Decimal(kauricoinlib.blocks_to_seconds(0)) == Decimal(0.0)
    assert Decimal(kauricoinlib.blocks_to_seconds(2)).quantize(precision) \
        == Decimal(314.4).quantize(precision)
    assert int(kauricoinlib.blocks_to_seconds(16616)) == 2612035
